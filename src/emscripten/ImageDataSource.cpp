...
inline char ImageDataSource::convertPixel(char const* pixel_) const {
...
ImageDataSource::ImageDataSource(ArrayRef<char> image_, int width, int height, int comps_)
    : Super(width, height), image(image_), comps(comps_) {}

Ref<LuminanceSource> ImageDataSource::create(char *data, int width, int height) {
  int comps = 4;
  zxing::ArrayRef<char> image;
  
  image = zxing::ArrayRef<char>(data, comps * width * height);
  
  return Ref<LuminanceSource>(new ImageDataSource(image, width, height, comps));
}

zxing::ArrayRef<char> ImageDataSource::getRow(int y, zxing::ArrayRef<char> row) const {
...
zxing::ArrayRef<char> ImageDataSource::getMatrix() const {
